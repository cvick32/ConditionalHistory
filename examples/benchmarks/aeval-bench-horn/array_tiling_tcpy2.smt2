...
(declare-var acopy0 (Array Int Int))
...
(rule (=> (and
           (inv a acopy i S)
           (< i (* 1 S))
           (= acopy0 (store acopy (- (* 5 S) (+ i 1)) (select a (- (* 5 S) (+ i 1)))))	   
           (= acopy1 (store acopy0 (- (* 4 S) (+ i 1)) (select a (- (* 4 S) (+ i 1)))))
...
(rule (=> (and (inv a acopy i S) (not (< i (* 1 S))) (<= 0 i1) (< i1 (* 5 S))
